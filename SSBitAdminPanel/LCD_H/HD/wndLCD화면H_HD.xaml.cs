...
{
	/// <summary>
	/// wndLCD화면H_HD.xaml에 대한 상호 작용 논리
	/// </summary>
	public partial class wndLCD화면H_HD : Window
	{
		public wndLCD화면H_HD()
		{
			InitializeComponent();
		...
		private void EPD_DisplayStartup()
		{
			...
			{
				case 0:
					if (++startDelay > 20)   //0 -> 10sec, for rPi too slow appl startup
						전송순서++;
					break;
				case 1:
					MainWindow.CaptureScreen(new Rect(0, 895, 720, 385)); // 하단 //edgar changed bottom capture
					전송순서++;
					break;
				case 2:
					MainWindow.CaptureScreen(new Rect(0, 105, 720, 130));  // 곧도착 전체 //edgar changed arriving soon capture
					전송순서++;
					break;
				case 3:
					MainWindow.CaptureScreen(new Rect(0, 235, 720, 660));  // 버스정보 전체 //edgar changed bus info capture
					전송순서++;
					break;
				case 4: 
					MainWindow.CaptureScreen(new Rect(0, 0, 720, 105));    // 상단화면 //edgar changed header capture
					MainWindow.startUpFine = true;
					전송순서++;
					break;
				default:
					//	DoFinal();
					//if (++startDelay > 60 * 60) //pjh 60분주기로 다시한번 전체 그림
					//{
					//	startDelay = 0;
					//	전송순서 = 0;
					//}
