...
namespace SSBitUI
{
	...
	/// </summary>
	public partial class uc상단화면H_HD : UserControl
	{
		public uc상단화면H_HD()
		{
			InitializeComponent();
		...
		private void NoWaitDoSomething()
		{
			...
			//MainWindow.CaptureScreen(name, new Rect(568, 50, 144, 40));
			if (MainWindow.startUpFine)
				MainWindow.CaptureScreen(new Rect(5, 60, 705, 40));   // 글자부분만 //edgar changed time, need to make size smaller capture
		}
		...
		//	if (MainWindow.startUpFine)
			//MainWindow.CaptureScreen(new Rect(520, 0, 200, 110));   // 글자부분만 //edgar no need to take weather pictures
		}
		private void TopDoSomething()
		{
			...
			//	if (MainWindow.startUpFine)
			//MainWindow.CaptureScreen(new Rect(0, 0, 720, 110));    // 상단화면 //edgar what is this for? capture
		}
