...
namespace BitView
{
        ...
        private void Application_Startup(object sender, StartupEventArgs e)
        {
            try
            {
                if (IsUseProcess() == false)
                {
                    ...
                    MessageBox.Show("중복실행으로 종료됩니다.", "중복실행으로 종료됩니다.", MessageBoxButton.OK);
				...
				string _tempDIR = RegisterManager.GET_APPS_DIR(프로그램Type.BIT);
                if (_tempDIR == null || _tempDIR.Equals("") == true)
                {
                    //시작폴더 설정
                    Current.ShutdownMode = System.Windows.ShutdownMode.OnExplicitShutdown;
                    wnd설치경로 _w폴더 = new wnd설치경로();
                    _w폴더.SetWindowStartLocation(WindowStartupLocation.CenterScreen);
                    if (_w폴더.ShowDialog().Value == true)
                    {
                        if (_w폴더 != null) _w폴더.Close();
                        ...
                        if (_fb.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                        {
                            ...
                            if (RegisterManager.SET_APPS_DIR(프로그램Type.BIT, AppConfig.APPSStartPath) == true)
                            {
                                Console.WriteLine("[REG] 프로그램 시작경로를 저장했습니다.");
                            }
                            else
                            {
                                Console.WriteLine("[REG] 프로그램 시작경로를 저장에 실패했습니다.");
                            }
                        }
                    }
                    else
                ...
                //프로그램 환경설정 체크
                if (DataManager.Initialize(프로그램Type.BIT) == false)
                {
                ...
                //edgar removed, we dont use DB for now
                //기본파일복사
                //CommonProc.Init기초파일();

                TraceManager.AddInfoLog(string.Format("BitView 프로그램을 시작합니다. v{0}", System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString()));
    ...
    //                wConfig.ShowDialog();
        ...
        private void Application_SessionEnding(object sender, SessionEndingCancelEventArgs e)
        {
            try
            {
                TraceManager.AddLog(e.ReasonSessionEnding.ToString());
